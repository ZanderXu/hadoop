...
public class AutoCloseableLockInPool<K> implements Closeable {
  ...
  public AutoCloseableLockInPool(K key, LockPoolManager<K> manager,
                                 LockResource<K> lock, LockMode mode) {
    this.key = key;
    ...
    if (mode == LockMode.WRITE && getReadHoldCount() > 0) {
      ...
      this.lockManager.releaseLockResource(key);
  ...
  public synchronized void close() {
    ...
    currentLock.unlock();
    currentLock = null;
    lockManager.releaseLockResourceAndRemoveFromLockTrace(key);
  }
