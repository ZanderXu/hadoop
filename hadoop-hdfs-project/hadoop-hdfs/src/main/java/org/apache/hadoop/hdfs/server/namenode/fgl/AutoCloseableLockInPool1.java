...
import org.apache.hadoop.hdfs.server.namenode.fgl.LockPoolManager1.LockMode;
 ...
 * </pre>
 *
 * Should not be manually constructed and should be managed by a {@link LockPoolManager1} (or
 * its wrappers) instead.
 * @param <K> lock key
 */
public class AutoCloseableLockInPool1<K> implements Closeable {

  /** The {@link LockPoolManager1} that handles this lock. */
  private final LockPoolManager1<K> lockManager;
  private final K lockKey;
  /** The underlying {@link LockResource1} used by this class. */
  private final LockResource1 lockResource;
  private LockMode lockMode;
  /**
   * The actual lock.
   * From {@link LockResource1#rwLock} depending on {@link AutoCloseableLockInPool1#lockMode}.
   */
  private Lock currentLock;

  public AutoCloseableLockInPool1(LockPoolManager1<K> manager, K lockKey, LockResource1 lock, LockMode mode) {
    ...
    if (mode == LockMode.WRITE && getReadHoldCount() > 0) {
      ...
      this.lockManager.releaseLockResource(this.lockKey);
  ...
  public synchronized void close() {
    ...
    currentLock.unlock();
    currentLock = null;
    lockManager.releaseLockResourceAndRemoveFromLockTrace(this.lockKey);
  }
